...
describe("PontoBlock", () => {
  async function setupFixture() {
    const [owner, billy, john, alice, jeff, rachel, employer] =
    ...
    const AdministratorDeployed = await AdministratorContract.deploy(
      taxId,
      name
    );
    ...
    const EmployerDeployed = await EmployerContract.deploy(
    ...
    await EmployerDeployed.addEmployer(
      emprAdd,
      emprTaxId,
      emprName,
      emprLegalAdd
    );
    const EmployeeContract = await ethers.getContractFactory(
      "EmployeeContract"
    );
    ...
    const EmployeeDeployed = await EmployeeContract.deploy(
      admAddress,
      emprAddress,
      utilAddress
    );
    ...
    await EmployeeDeployed.addEmployee(
      billy.address,
      nameEmp1,
      taxIdEmp1,
      beggining,
    ...
    const inactiveEmployee = await EmployeeDeployed.getEmployeeByAddress(
      rachel.address
    );
    await EmployeeDeployed.updateEmployee(
    ...
    const PontoBlockDeployed = await PontoBlock.deploy(
      EmployeeDeployed.address,
  ...
  describe("Add a start of work", () => {
    it("should start of work", async () => {
      const { PontoBlockDeployed, UtilDeployed, john } = await loadFixture(
        setupFixture
      );
      ...
      const record = await (
        await PontoBlockDeployed.getEmployeeRecords(john.address, contractDate)
      ).startWork;
    ...
    it("should not start work - Employee not registered", async () => {
      ...
      await expect(
        PontoBlockDeployed.connect(jeff).startWork()
    ...
    it("should not start work - Employee is inactive", async () => {
      ...
      await expect(
        PontoBlockDeployed.connect(rachel).startWork()
  ...
  describe("Add an end of work", () => {
    it("should end of work", async () => {
      const { PontoBlockDeployed, UtilDeployed, billy } = await loadFixture(
        setupFixture
      );
      ...
      const contractDate = await UtilDeployed.getDate(contractTime);
      const record = await (
        await PontoBlockDeployed.getEmployeeRecords(billy.address, contractDate)
      ).endWork;
      expect(record.toNumber() > 0).to.true;
    });
    ...
    it("should not end of work - Employee not registered", async () => {
      const { PontoBlockDeployed, jeff } = await loadFixture(setupFixture);
      await expect(PontoBlockDeployed.connect(jeff).endWork()).to.rejectedWith(
    ...
    it("should not end work - Employee is inactive", async () => {
      const { PontoBlockDeployed, rachel } = await loadFixture(setupFixture);
      await expect(
        PontoBlockDeployed.connect(rachel).endWork()
      ).to.rejectedWith("Employee is inactive.");
    });
    it("should not end of work - End of work already registered.", async () => {
      ...
      await expect(PontoBlockDeployed.connect(john).endWork()).to.rejectedWith(
        "End of work already registered."
      );
    });
    it("should not end of work - Start of work not registered.", async () => {
      const { PontoBlockDeployed, john } = await loadFixture(setupFixture);
      await expect(PontoBlockDeployed.connect(john).endWork()).to.rejectedWith(
        "Start of work not registered."
      );
    });
  });
  describe("Add an break start time", () => {
    ...
    it("should not break start time - Employee not registered", async () => {
      const { PontoBlockDeployed, jeff } = await loadFixture(setupFixture);
      await expect(
        PontoBlockDeployed.connect(jeff).breakStartTime()
      ).to.rejectedWith("Employee not registered.");
    });
    it("should not break start time - Employee is inactive", async () => {
      const { PontoBlockDeployed, rachel } = await loadFixture(setupFixture);
      await expect(
        PontoBlockDeployed.connect(rachel).breakStartTime()
      ).to.rejectedWith("Employee is inactive.");
    });
    it("should not break start time - Start of break already registered", async () => {
      ...
      await expect(
        PontoBlockDeployed.connect(john).breakStartTime()
  ...
  describe("Add an break end time", () => {
    ...
    it("should not break end time - Employee not registered", async () => {
      const { PontoBlockDeployed, jeff } = await loadFixture(setupFixture);
      await expect(
        PontoBlockDeployed.connect(jeff).breakEndTime()
      ).to.rejectedWith("Employee not registered.");
    });
    it("should not break end time - Employee is inactive", async () => {
      const { PontoBlockDeployed, rachel } = await loadFixture(setupFixture);
      await expect(
        PontoBlockDeployed.connect(rachel).breakEndTime()
      ).to.rejectedWith("Employee is inactive.");
    });
    it("should not break end time - End of break already registered", async () => {
      ...
      await PontoBlockDeployed.connect(john).breakEndTime();
      await expect(
        PontoBlockDeployed.connect(john).breakEndTime()
  ...
  describe("get employee records", () => {
    ...
    it("should not get employee records - Sender is not administrator", async () => {
      const { PontoBlockDeployed, UtilDeployed, john, rachel } =
      ...
      const contractDate = await UtilDeployed.getDate(contractTime);
      await expect(
        PontoBlockDeployed.connect(rachel).getEmployeeRecords(
          john.address,
          contractDate
        )
  ...
  describe("get a contract date", () => {
    it("should get a contract date", async () => {
      const { PontoBlockDeployed, UtilDeployed } = await loadFixture(
        setupFixture
      );
  ...
  describe("Get owner", () => {
    it("should get the owner", async () => {
      const { PontoBlockDeployed, owner } = await loadFixture(setupFixture);
  ...
  describe("Change owner", () => {
    it("should change the owner", async () => {
      const { PontoBlockDeployed, owner, alice } = await loadFixture(
        setupFixture
      );
    ...
    it("should not change the owner - Only owner", async () => {
      const { PontoBlockDeployed, rachel, alice } = await loadFixture(
        setupFixture
      );
      await expect(
        PontoBlockDeployed.connect(alice).changeOwner(rachel.address)
      ).to.rejectedWith("Only the owner can call this function.");
    });
  });
  describe("Getting Time Zone", () => {
    it("should get Time Zone", async () => {
      const { PontoBlockDeployed, minTimeZone, maxTimeZone } =
        await loadFixture(setupFixture);
